...
if no_of_items > 0:
    ...
    print('************List 3 items*************')
    for k in range(1,no_of_items + 1):
        list_item=int(input(f'Enter item {k}: \n'))
        myList3.append(list_item)
else:
...
for item in range(no_of_items):
    final_list.append(min(min(myList1[item],myList2[item]),myList3[item]))
